...
def test_a_wheel_survives_a_round_trip(db):
    ...
    assert store.load_position("SPY") == original


def test_an_unknown_symbol_starts_in_cash(db):
    fresh = store.load_position("QQQ")
...
def test_saving_the_same_symbol_twice_updates_rather_than_duplicates(db):
    store.save_position(Position(symbol="SPY", leg="CASH"))
    store.save_position(Position(symbol="SPY", leg="PUT_OPEN"))
    assert store.load_position("SPY").leg == "PUT_OPEN"
    assert list(store.load_all()) == ["SPY"]


def test_load_all_returns_every_saved_wheel(db):
    store.save_position(Position(symbol="SPY", leg="PUT_OPEN"))
    store.save_position(Position(symbol="IWM", leg="SHARES", shares=100))
...
def test_a_snapshot_restores_state_into_a_fresh_database(db, tmp_path):
    ...
    assert store.load_position("SPY").leg == "CASH"  # genuinely empty first
...
def test_the_snapshot_preserves_decimal_precision(db, tmp_path):
    ...
    """
    store.save_position(Position(symbol="SPY", basis=Decimal("472.70")))
    snapshot = tmp_path / "state" / "positions.json"
    store.export_snapshot(snapshot)
    store.init_db(tmp_path / "second.db")
    store.import_snapshot(snapshot)

    restored = store.load_position("SPY").basis
...
def test_the_snapshot_is_written_in_a_stable_order(db, tmp_path):
    ...
    """
    store.save_position(Position(symbol="SPY"))
    store.save_position(Position(symbol="IWM"))
    store.save_position(Position(symbol="QQQ"))
    snapshot = tmp_path / "state" / "positions.json"
    store.export_snapshot(snapshot)
...
def test_init_db_seeds_itself_from_a_snapshot_when_empty(db, tmp_path):
    ...
    store.save_position(wheel_holding_shares())
    snapshot = tmp_path / "state" / "positions.json"
    store.export_snapshot(snapshot)

    store.init_db(tmp_path / "third.db", snapshot=snapshot)
    assert store.load_position("SPY") == wheel_holding_shares()
...
def test_init_db_does_not_overwrite_a_populated_database(db, tmp_path):
    ...
    snapshot = tmp_path / "state" / "positions.json"
    store.save_position(Position(symbol="SPY", cycle_count=1))
    store.export_snapshot(snapshot)
    store.save_position(Position(symbol="SPY", cycle_count=9))

    store.init_db(tmp_path / "drawdownguard.db", snapshot=snapshot)
    assert store.load_position("SPY").cycle_count == 9
